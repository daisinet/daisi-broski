...
**Phases 0, 1, 3a, 3b, and 4 are shipped. Phase 3c is in progress — slice 3c-1 added the ES2020 short-circuit operators (`?.`, `??`, `&&=`, `||=`, `??=`); slice 3c-2 wires the DOM to JS so scripts can query, walk, and mutate the real parsed document; slice 3c-3 ships the WHATWG web-primitive built-ins (`URL`, `URLSearchParams`, `TextEncoder`, `TextDecoder`, `atob`, `btoa`) — the foundation a future `fetch` slice will build on.**
...
**Combined test suite: 1224/1224 passing.** Phase 3b (ES2015+) is complete; phase 3c is underway. All engine, DOM, selector, and JS tests run in under a few seconds; the sandbox and CLI integration tests spawn real child processes against a local `HttpListener` fixture.
