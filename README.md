...
**Combined test suite: 1582/1582 passing.** Phases 3a–3c are all complete, plus regex literals, two rounds of browser host shims, the ES2015+ prototype additions, and the DOM ChildNode/ParentNode mixin methods (`append` / `prepend` / `remove` / `before` / `after` / `replaceChildren`) + `document.currentScript` tracking + URL base-object coercion. **svelte.dev now runs 6/6 of its inline bootstrap scripts cleanly** — the phase 3 ship gate (load a scripted page, run the scripts, produce the expected post-script DOM) is demonstrably working on a real modern SPA homepage. All engine, DOM, selector, and JS tests run in under a few seconds; the sandbox and CLI integration tests spawn real child processes against a local `HttpListener` fixture.
