...
**Combined test suite: 508/508 passing** (152 engine phase-1 + 12 IPC codec + 7 Job Object + 4 sandbox integration + 5 CLI smoke + 43 JS lexer + 69 JS parser + 51 JS VM + 38 JS objects + 34 JS functions + 25 JS control flow + 22 JS exceptions + 46 JS built-ins).
...
- Built-in library, slice 6b — `Object`, `Math`, `JSON`, `Error` constructors, and the callback-taking `Array` methods (`forEach`, `map`, `filter`, `reduce`, `every`, `some`, `sort` with compareFn). The callback-taking methods need a re-entrant VM dispatch loop so native code can invoke a JS function synchronously — that's the main new infrastructure for 6b.
- Built-in library, slice 6c — `Number`, `Boolean`, `Function.prototype.call` / `apply` / `bind`, `Date`.
...
**Current status:** phase 3a in progress — lexer, parser, bytecode VM, objects, arrays, member access, full function / closure / `this` / `new` / `instanceof` system, the remaining ES5 control flow, exception handling, and the first slice of the built-in library (`parseInt`/`parseFloat`/`isNaN`/`isFinite`, `Array` + prototype, `String` + prototype) are shipped. The rest of the built-ins (`Object`, `Math`, `JSON`, `Error`, callback-taking `Array` methods, `Number`, `Boolean`, `Date`) and the event loop are still ahead.
