...
**Combined test suite: 462/462 passing** (152 engine phase-1 + 12 IPC codec + 7 Job Object + 4 sandbox integration + 5 CLI smoke + 43 JS lexer + 69 JS parser + 51 JS VM + 38 JS objects + 34 JS functions + 25 JS control flow + 22 JS exceptions; the VM count dropped by 1 after a slice-4 "throws compile error" negative test was superseded by real support).
...
- **Exception handling — `throw` / `try` / `catch` / `finally` (slice 5)** ✅ — New opcodes `PushCatchHandler` / `PushFinallyHandler` (register a handler at a relative offset, saving stack depth, env, and frame depth), `PopHandler`, `Throw`, `EndFinally`, `PushEnv` / `PopEnv` (for catch-parameter block scoping). The VM carries a `Stack<HandlerRecord>` and an `_pendingException` slot. `DoThrow` walks the handler stack, unwinds call frames one by one when no handler applies in the current frame, and either activates a catch (pushes the thrown value onto the stack and jumps) or routes through a finally-only handler (sets `_pendingException` and jumps; `EndFinally` checks the slot and re-throws on exit). `Return` cleans up handlers belonging to the returning frame. All internal VM errors route through a `RaiseError(name, message)` helper that builds a plain `JsObject` with `name` and `message` properties and throws it, so script-level `try { undeclared; } catch (e) { e.name }` sees `"ReferenceError"`. Uncaught throws escape the VM as a .NET `JsRuntimeException` carrying the thrown `JsValue` for host inspection. `try / catch / finally` uses three code layouts (catch-only, finally-only, catch+finally); catch+finally installs a finally-only handler around the catch body so a throw during catch still runs finally. Finally body is inlined twice or thrice depending on shape (normal + exception paths). 22 end-to-end tests covering primitive / string / object throws, catch-parameter block scoping, rethrow, uncaught escape to host, nested try, frame unwinding across function calls, catchable `ReferenceError` / `TypeError` from undeclared reads and bad property access, try/finally normal path, try/finally exception path, try/catch/finally with throw during catch. **Deferred**: the cross-`finally` escape semantics for `return` / `break` / `continue` (slice 5b or later) — for now, those statements skip the finally body, which is technically spec-incorrect but never comes up in the common-case try/catch/cleanup pattern.
...
**Current status:** phase 3a in progress — lexer, parser, bytecode VM, objects, arrays, member access, full function / closure / `this` / `new` / `instanceof` system, the remaining ES5 control flow (`for..in`, `switch`, labeled break/continue), and exception handling (`throw` / `try` / `catch` / `finally`) are all shipped. The built-in library (`Object`, `Array`, `String`, `Math`, `JSON`, `Error`) and the event loop are still ahead.
