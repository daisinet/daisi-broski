...
**Shipped so far (phase 3a slices 1–6a):**
...
- **Built-in library, slice 6a — globals + Array + String** — `JsEngine` now constructs well-known prototype objects (`ObjectPrototype`, `ArrayPrototype`, `StringPrototype`, `NumberPrototype`, `BooleanPrototype`) and installs built-ins at startup via `Builtins.Install`. Native built-in methods are plain C# delegates installed as non-enumerable properties on the prototype objects (`JsObject.SetNonEnumerable` plus `JsObject.OwnKeys` filtering via a `_nonEnumerable` set, so `for..in` doesn't walk over `Array.prototype.push` etc.). Native methods signal catchable JS exceptions by throwing a `JsThrowSignal` via the `JsThrow.TypeError` / `RangeError` / `SyntaxError` helpers; `InvokeFunction` catches the signal and routes it through `DoThrow`. The VM takes a `JsEngine` reference so `CreateArray` can set the array prototype and `GetProperty` / `GetPropertyComputed` can resolve string-primitive property access through `StringPrototype` (including `length` and integer indexing). Slice 6a ships: **globals** `parseInt`, `parseFloat`, `isNaN`, `isFinite`; **Array** constructor + `Array.isArray` static + prototype methods `push`, `pop`, `shift`, `unshift`, `slice`, `concat`, `join`, `indexOf`, `reverse`, `sort` (no compareFn), `toString`; **String** coercion constructor + `String.fromCharCode` static + prototype methods `charAt`, `charCodeAt`, `indexOf`, `lastIndexOf`, `slice`, `substring`, `substr`, `toLowerCase`, `toUpperCase`, `trim`, `split`, `concat`, `toString`. Callback-taking array methods (`forEach`, `map`, `filter`, `reduce`, `sort` with compareFn) defer to slice 6b because they need a re-entrant VM loop to call JS from native. 46 end-to-end tests including chained calls like `'hello'.split('').reverse().join('')`.
