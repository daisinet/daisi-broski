...
**Shipped so far (phase 3a slices 1–6c):**
...
- **Built-in library, slice 6c — JSON + Function.prototype + Number/Boolean** — `BuiltinJson` implements `JSON.parse` as a recursive-descent parser over the JSON grammar (with string escapes, `\uXXXX` unicode, scientific notation, proper `SyntaxError` on malformed input) and `JSON.stringify` as a recursive walker with reference-equality cycle detection (throws `TypeError` on cycles), NaN/Infinity → `"null"`, and functions/undefined omitted from objects / rendered as `null` inside arrays. `BuiltinFunction` installs `call`, `apply`, and `bind` on `JsEngine.FunctionPrototype` — the new `[[Prototype]]` for every `JsFunction` value. `JsEngine` sets `FunctionPrototype` on user functions via the `MakeFunction` opcode and fixes up built-in functions via a post-install prototype-chain walk. `bind` returns a fresh `NativeCallable` that closes over the target function, bound `this`, and prefix arguments, merging them with runtime args on every invocation. `BuiltinNumberBoolean` ships `Number` constructor, static constants (`MAX_VALUE`, `MIN_VALUE`, `NaN`, `POSITIVE_INFINITY`, `NEGATIVE_INFINITY`, `EPSILON`, `MAX_SAFE_INTEGER`, `MIN_SAFE_INTEGER`), static methods (`Number.isNaN` / `isFinite` / `isInteger` — strict-typed, don't coerce), and prototype methods (`toString(radix)` with base-2-to-36 integer conversion, `toFixed(digits)`, `valueOf`). `Boolean` gets a coercion constructor plus `toString` / `valueOf`. 39 end-to-end tests including JSON roundtrips, cycle detection, radix conversion, `bind` composing with `forEach`, and `Number` statics.
