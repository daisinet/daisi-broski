...
**Shipped so far (phase 3a slices 1–4b):**
...
- **Functions + closures + `this` + `new` + `instanceof` (slice 4b)** — `JsEnvironment` (Dictionary-backed binding record with a parent reference) replaces the single globals dictionary as the name-resolution mechanism. `LoadGlobal` and friends now walk the env chain; at the top level the chain has one env (the globals env), so existing behavior is preserved. `JsFunctionTemplate` is the immutable compiled form of a function (chunk + param names); `JsFunction` subclasses `JsObject` to carry a template plus a captured env reference plus an auto-initialized `prototype` object. The VM grew a call-frame stack, a current `_env` pointer, and a `_this` slot; `Call` / `New` opcodes set up a fresh env with parameters bound, push a frame, and switch execution to the callee; `Return` pops the frame, handles the constructor-return rule from ECMA §13.2.2, and resumes execution. Method calls route through a `Dup` / `GetProperty` / `Swap` sequence so the call instruction sees the canonical `[fn, this, args...]` stack layout. `new` allocates a fresh instance, links its prototype to `F.prototype` read live so `Dog.prototype = new Animal()` takes effect for subsequent instances. `instanceof` walks the object's prototype chain looking for the constructor's live `prototype`. `arguments` binds to a `JsArray` of the raw argument values. Host-installed native functions attach via a `NativeImpl` delegate — `engine.Globals["log"] = new JsFunction("log", (self, args) => ...)` works end-to-end from script and is how slice 6 will wire up `console`, `Math`, and the rest of the built-ins. The compiler grew a frame stack so nested function bodies get their own `Chunk`; function declarations are fully hoisted (both the binding AND the value) so `f(); function f() {...}` runs. 34 end-to-end tests including recursive factorial / Fibonacci, independent-state counter closures, method calls with and without `this` loss on detachment, `new` with prototype chains, `instanceof` through an inherited prototype, constructor-return-object overrides, `arguments.length`, host native function callbacks.
