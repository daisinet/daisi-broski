...
**Shipped so far (phase 3a slices 1–6b):**
...
- **Built-in library, slice 6b — re-entrant VM + Object + Math + Error + callback Array methods** — `JsVM.Run` is refactored to delegate to `RunLoop(stopFrameDepth)` plus a `_halted` flag, and exposes a public `InvokeJsFunction(fn, this, args)` that native built-ins call to run a JS function synchronously. The inside of `InvokeJsFunction` sets up a call (via `DoCall`), runs a nested dispatch until the pushed frame is popped, and returns the return value. A `_nativeBoundaries` stack records frame depths below which `DoThrow` cannot safely unwind; when a throw would unwind past one, `DoThrow` instead raises a `JsThrowSignal` that the calling native's wrapper catches — the wrapper cleans up abandoned frames / handlers, restores the caller's `_code`/`_ip`/`_env`/`_this`/`_sp` to a snapshot taken at `InvokeJsFunction` entry, and re-throws. The outer `InvokeFunction` catches the re-thrown signal at its usual native-result path and re-enters `DoThrow` from the now-safe context. `JsFunction` grew a second native-implementation delegate `NativeCallable` that takes a `JsVM` reference, so built-ins that need callback dispatch (`forEach`, `map`, `filter`, `reduce`, `reduceRight`, `every`, `some`, `sort` with compareFn) can reach the `InvokeJsFunction` helper. Slice 6b installs: **Object** constructor plus `Object.keys` / `Object.create` / `Object.getPrototypeOf` statics and `Object.prototype.hasOwnProperty` / `isPrototypeOf` / `propertyIsEnumerable` / `toString` / `valueOf`; **Math** with the standard constants (`PI`, `E`, `LN2`, `LN10`, `LOG2E`, `LOG10E`, `SQRT2`, `SQRT1_2`) and methods (`abs`, `ceil`, `floor`, `round` matching ECMA's round-half-up rule, `sqrt`, `exp`, `log`, `sin`, `cos`, `tan`, `asin`, `acos`, `atan`, `sign`, `pow`, `atan2`, `min` / `max` variadic, `random`); **Error hierarchy** — `Error`, `TypeError`, `RangeError`, `SyntaxError`, `ReferenceError`, `EvalError`, `URIError`, each with a prototype carrying `name` / `message` / `toString`, subclasses chained to `Error.prototype`. The VM's `RaiseError` now tags internal errors with the right prototype (via new engine fields `ErrorPrototype` / `TypeErrorPrototype` / `RangeErrorPrototype` / `SyntaxErrorPrototype` / `ReferenceErrorPrototype`) so `e instanceof TypeError` works for catches of VM-originated errors too. 41 end-to-end tests including `filter`/`map`/`reduce` pipelines, `sort` with compareFn, callback-throw propagation through `forEach` to an outer try/catch, `Math.min()` / `Math.max()` no-arg edge cases, and `ReferenceError` / `TypeError` instanceof checks from script.
