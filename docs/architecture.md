...
**Shipped so far (phase 3a and 3b both complete):**
...
- **ESM module loader (slice 3b-12)** — New AST node trio `ImportDeclaration` / `ExportNamedDeclaration` / `ExportDefaultDeclaration` (with supporting `ImportSpecifier` / `ExportSpecifier`). Parser accepts `import`/`export` only at source-element level via a new `ParseImportDeclaration` / `ParseExportDeclaration` pair, with both `as` and `from` as contextual keywords. `JsModule` (in `JsModule.cs`) holds the resolved URL, an `Exports` JsObject serving as the namespace, and a three-state `ModuleState` for future circular-import work. `JsEngine.ModuleResolver` is a host-pluggable delegate mapping (specifier, referrer) → (url, source); `JsEngine.ImportModule` is the top-level loader. The loader flow: resolve → cache-check → parse → recursively load the dependency graph (breadth-first, collecting each dependency's exports into a local `importBindings` map) → compile via the new `JsCompiler.CompileModule` path → `JsVM.RunModuleChunk`. `CompileModule` is a pragmatic wrapper that reuses the existing program-compile path but **defers all named-export copies (`$exports.name = name`) until the end of the module body**, so top-level mutations during initialization are reflected in the exports snapshot. `RunModuleChunk` temporarily swaps in a fresh `JsEnvironment` chained above the engine's globals, pre-seeds it with the loader-resolved imports plus a synthetic `$exports` binding pointing at the module's `Exports` JsObject, runs the chunk to completion via `RunLoop`, and restores the caller's env on exit. The compiler's statement dispatch recognizes `ImportDeclaration` (no-op at emit time — the loader pre-seeded the bindings), `ExportNamedDeclaration` (compile the wrapped declaration normally, add the name to the deferred-exports list), and `ExportDefaultDeclaration` (emit an inline `$exports.default = value` store; for function and class default exports, the wrapped declaration is compiled first so its local binding stays usable inside the module). 19 end-to-end tests including all the common import / export forms, default exports of values / functions / classes (named and anonymous), namespace imports, rename, side-effect imports, transitive imports, module caching (body runs exactly once, shared references across importers), class-export-with-new, and promise integration across modules. Deferred: dynamic `import()`, re-exports (`export { a } from './b'`, `export * from ...`), live-binding semantics (we snapshot at end-of-module), top-level `await`, and `import.meta`.
