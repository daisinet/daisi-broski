...
**Shipped so far (phase 3a slices 1–5):**
...
- **Exception handling — `throw` / `try` / `catch` / `finally` (slice 5)** — New opcodes `PushCatchHandler` / `PushFinallyHandler` (register a handler at a relative offset, saving stack depth, env, and frame depth), `PopHandler`, `Throw`, `EndFinally`, `PushEnv` / `PopEnv`. The VM carries a `Stack<HandlerRecord>` and a `_pendingException` slot. `DoThrow` walks the handler stack, unwinds call frames one by one when no handler applies in the current frame, and activates the nearest catch (pushing the thrown value) or finally-only handler (setting `_pendingException` for `EndFinally` to re-throw). `Return` cleans up handlers belonging to the returning frame. All internal VM errors now route through a `RaiseError(name, message)` helper that constructs a plain `JsObject` with `name` and `message` and calls `DoThrow`, so `try { undeclared; } catch (e) { e.name }` yields `"ReferenceError"` from script. Uncaught throws escape the VM as a .NET `JsRuntimeException` carrying the thrown JS value for host inspection via the new `JsValue` property. The compiler emits three different `try` layouts — catch-only, finally-only, catch+finally — each duplicating the finally body at each exit point rather than using a subroutine convention; catch+finally installs a finally-only handler around the catch body so a throw during catch still runs finally. The catch parameter gets its own `JsEnvironment` via `PushEnv` so it doesn't leak into the enclosing function. `return`/`break`/`continue` crossing a `finally` is documented as deferred (those statements currently skip the finally body; spec-correct completion-value tracking is a later refinement). 22 end-to-end tests.
